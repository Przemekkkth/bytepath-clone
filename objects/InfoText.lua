...
function InfoText:new(area, x, y, opts)
    ...
    local all_info_texts = self.area:getAllGameObjectsThat(
        function(o) 
            if o:is(Ammo) and o.id ~= self.id then 
                return true 
            end 
        end)
    local collidesWithOtherInfoText = function()
        for _, info_text in ipairs(all_info_texts) do
            return areRectanglesOverlapping(self.x, self.y, self.x + self.width, self.y + self.height, info_text.x, info_text.y, info_text.x + info_text.width, info_text.y + info_text.height)
        end
    end
    while collidesWithOtherInfoText() do
        self.x = self.x + table.random({-1, 0, 1})*self.width
        self.y = self.y + table.random({-1, 0, 1})*self.height
    end
