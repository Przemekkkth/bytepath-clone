...
function PreAttackEffect:update(dt)
    ...
    if self.shooter and not self.shooter.dead then
        self.x, self.y = self.shooter.x + 1.4*self.shooter.width*math.cos(self.shooter.collider:getAngle()), self.shooter.y + 1.4*self.shooter.width*math.sin(self.shooter.collider:getAngle())
    end
end
